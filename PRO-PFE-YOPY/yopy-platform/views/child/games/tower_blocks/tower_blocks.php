<?php
// views/child/games/tower_blocks.php
// YOPY Tower Blocks | Category: Strategy / Reaction
...
<head>
    ...
    <title>YOPY — Tower Blocks</title>

    <!-- Three.js + GSAP -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r134/three.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.5/gsap.min.js"></script>

    <link rel="stylesheet" href="./tower_blocks.css">
</head>
<body>
...
<script src="../../../../public/js/games/BehaviorCollector.js"></script>
    ...
    // ==================== PHP → JS Bridge + Standard BehaviorCollector ====================
    const GAME_ID      = 'tower_blocks';
    const SESSION_ID   = '<?= $sessionId ?>';
    const BUDDY_ID     = '<?= $buddyId ?>';

    const BUDDY_EMOJIS = {joy:'😊',sadness:'😢',anger:'😠',disgust:'🤢',fear:'😨',anxiety:'😰'};
    document.getElementById('buddy-av').textContent = BUDDY_EMOJIS[BUDDY_ID] || '😊';

    // ── Standard BehaviorCollector (same as all other games) ──
    const SIGNAL_MAP = {
        move:      'reaction',   // block is moving
        place:     'success',    // successfully placed block
        miss:      'error',      // missed / chopped block
        game_over: 'error'
    };

    let collector = new BehaviorCollector(GAME_ID, SIGNAL_MAP);

    let selectedDifficulty = 'easy';
    let towerGameInstance = null;
    let gameStartTime = 0;
    let gameTimerInterval = null;
    let serverSessionId = SESSION_ID;   // will be updated by backend
    ...
    // Create session + get real session_id
    try {
        const res = await fetch('../../../../games/tower_blocks/tower_blocks_backend.php?action=start', {
            method: 'POST',
        ...
        if (!contentType.includes('application/json')) {
            console.warn('[Session] Unexpected response type:', contentType);
        } else {
    ...
    if (!towerGameInstance) {
        towerGameInstance = new Game();
    } else {
        towerGameInstance.restartGame();
    }
    towerGameInstance.startGame();
}

    function resetGame() {
        stopTimer();
        startTimer();
        if (towerGameInstance) towerGameInstance.restartGame();
    }
   ...
   async function submitScore(points, completionTime) {
    ...
    try {
        const res = await fetch('../../../../games/tower_blocks/tower_blocks_backend.php?action=score', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        const contentType = res.headers.get('content-type') || '';
        if (!contentType.includes('application/json')) {
            console.warn('[Score] Unexpected response type:', contentType);
            return;
        }
        ...
        var Block = /** @class */ (function () {
            function Block(block) {
                ...
                // Difficulty speed adjustment - much more noticeable differences
                let speedMultiplier = 1.0;
                if (selectedDifficulty === 'medium') speedMultiplier = 1.8;
                if (selectedDifficulty === 'hard') speedMultiplier = 2.5;
                this.speed = (-0.1 - (this.index * 0.005)) * speedMultiplier;
...
var Game = /** @class */ (function () {
    function Game() {
        ...
        var baseBlock = new Block(null);
        this.baseBlockMesh = baseBlock.mesh;
        this.placedBlocks.add(this.baseBlockMesh);
        ...
        document.addEventListener('click', function (e) { 
            if(!e.target.closest('.ctrl-btns') && !e.target.closest('.topbar') && !e.target.closest('#screen-difficulty')) {
                _this.onAction(); 
            }
        });
        document.addEventListener('touchstart', function (e) { 
            if(!e.target.closest('.ctrl-btns') && !e.target.closest('.topbar') && !e.target.closest('#screen-difficulty') && !e.target.closest('.game-overlay')) {
                e.preventDefault(); 
                _this.onAction();
            }
        }, {passive: false});
            ...
            Game.prototype.restartGame = function () {
                ...
                var _loop_1 = function (i) {
                    // Skip the base block – we don't want to remove or animate it
                    if (oldBlocks[i] === _this.baseBlockMesh) return;
                ...
                TweenLite.to(countdown, cameraMoveSpeed, { value: 0, onUpdate: function () { 
                    ...
                    var blocksEl = document.getElementById('hud-blocks');
                    if(blocksEl) blocksEl.innerText = String(Math.round(countdown.value));
                    var scoreEl = document.getElementById('tb-score');
                    if(scoreEl) scoreEl.innerText = '0';
                } });
                // Clear blocks except the base block (index 0)
                this.blocks = this.blocks.slice(0, 1);
                // Remove existing moving block from newBlocks group
      ...
      Game.prototype.placeBlock = function () {
            ...
            if (newBlocks.placed) {
                ...
                if (collector) collector.record('place', null);   // SUCCESS
            }
            if (newBlocks.chopped) {
                this.choppedBlocks.add(newBlocks.chopped);
                if (collector) collector.record('miss', null);    // ERROR
        ...
        let blockCount = this.blocks.length - 1;
        this.scoreContainer.innerHTML = String(blockCount);

        let multiplier = selectedDifficulty === 'hard' ? 40 : (selectedDifficulty === 'medium' ? 20 : 10);
        let finalScore = blockCount * multiplier;

        const blocksEl = document.getElementById('hud-blocks');
        if(blocksEl) blocksEl.innerText = blockCount;
        const scoreEl = document.getElementById('tb-score');
        if(scoreEl) scoreEl.innerText = finalScore;

        if (collector) collector.record('move', blockCount);
    ...
    Game.prototype.endGame = function () {
            ...
            let blockCount = this.blocks.length - 2;
            if (blockCount < 0) blockCount = 0;

            let multiplier = selectedDifficulty === 'hard' ? 40 : (selectedDifficulty === 'medium' ? 20 : 10);
            let finalScore = blockCount * multiplier;
            let completionTime = Math.floor((Date.now() - gameStartTime) / 1000);

            document.getElementById('final-score-msg').innerHTML = `Vous avez posé ${blockCount} bloc${blockCount>1?'s':''} !<br>Score : ${finalScore}`;
            document.getElementById('final-time-msg').innerText = `Temps : ${completionTime}s`;
            document.getElementById('game-overlay').classList.add('show');

            if (collector) {
                collector.record('game_over', finalScore);
                collector.flush();
            }

            submitScore(finalScore, completionTime);
        };
